...
export default function HomePage() {
  ...
  return (
    <div className="space-y-8 pb-20">
      ...
      <div className="flex items-center gap-2 text-[11px] font-medium text-zinc-400 uppercase tracking-widest px-1">
      ...
      <AnimatePresence mode="wait">
        ...
        >
          {activeTab === 'monitor' && <OperationsMonitor />}
          {activeTab === 'creator' && <AgentCreator />}
          {activeTab === 'vault' && <ConfigVault />}
        </motion.div>
      </AnimatePresence>
    </div>
  )
  ...
  return (
    <div className="space-y-10">
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
        <MiniStat label="Active Agents" value="12" delta="+2" />
        <MiniStat label="Total Messages" value="124.5k" delta="+15%" />
        <MiniStat label="Latency" value="142ms" status="Optimal" />
        <MiniStat label="Uptime" value="99.98%" status="Stable" />
      </div>
      ...
      <div className="space-y-4">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 xl:grid-cols-3 gap-6">
          ...
          <AgentStatusCard name="Prototype Z" status="created" type="Research" />
        </div>
      </div>
    </div>
  )
...
function MiniStat({ label, value, delta, status }: any) {
  return (
    <div className="glass-card p-5 space-y-2">
      <span className="premium-label text-[10px]">{label}</span>
      <div className="flex items-end justify-between">
        <h4 className="text-xl font-bold text-zinc-900">{value}</h4>
        {delta && <span className="text-[10px] font-black text-blue-600 bg-blue-50 px-2 py-0.5 rounded-full">{delta}</span>}
        {status && <span className="text-[10px] font-black text-emerald-600 bg-emerald-50 px-2 py-0.5 rounded-full">{status}</span>}
      </div>
    </div>
  )
}
      ...
      <div className="lg:col-span-4 space-y-6">
        <div className="glass-card p-8 space-y-4 bg-zinc-900 text-white">
          ...
          <div className="space-y-3 pt-4 border-t border-zinc-800">
             <StepLabel active number={1} label="Identidad Nuclear" />
             <StepLabel number={2} label="Configuración de Motor" />
             <StepLabel number={3} label="Inyección de Conocimiento" />
             <StepLabel number={4} label="Activación Final" />
          </div>
        </div>
      </div>
      ...
      <div className="lg:col-span-8 glass-card p-10 bg-white">
        <div className="max-w-xl space-y-10">
          <div className="space-y-2">
            <label className="text-[11px] font-black uppercase text-zinc-400 tracking-widest">Nombre del Agente</label>
            <input 
              type="text" 
          ...
          <div className="space-y-2">
            ...
            <div className="grid grid-cols-2 gap-4">
               ...
               <button className="p-4 border-2 border-zinc-100 hover:border-zinc-200 rounded-xl flex items-center gap-3 grayscale hover:grayscale-0 transition-all">
          ...
          <div className="pt-6 border-t border-zinc-100 flex justify-end">
            <button className="nuclear-button !px-10 !py-4 text-sm group">
...
function StepLabel({ number, label, active = false }: any) {
  return (
    <div className={`flex items-center gap-3 ${active ? 'text-white' : 'text-zinc-600'}`}>
      <div className={`w-6 h-6 rounded-full flex items-center justify-center text-[10px] font-bold border ${active ? 'bg-blue-600 border-blue-600' : 'border-zinc-700'}`}>
        {number}
      </div>
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="glass-card bg-white overflow-hidden divide-y divide-zinc-100">
        <FileRow name="agent_config.json" type="JSON" size="4.2kb" date="Mar 25, 2026" />
        <FileRow name="instruction_set.md" type="Markdown" size="12.8kb" date="Mar 25, 2026" />
        <FileRow name="deployment_guide.pdf" type="Manual" size="440kb" date="Mar 24, 2026" />
        <FileRow name="nuclear_protocol_v2.json" type="System" size="1.2mb" date="Mar 23, 2026" />
      </div>
